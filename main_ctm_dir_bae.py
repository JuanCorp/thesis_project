...
def run_experiment():
    ...
    utils = Evaluation()
    utils.create_utility_objects(test_bow_prepped_sp)
    ...
    utils.plot_kl_divergence(teacher_posterior,student_posterior)

    teacher_topics = model.predict(test_dataset)
    teacher_utils = Evaluation()
    teacher_utils.create_utility_objects(test_bow_prepped)
    teacher_top_tokens = teacher_utils.get_top_topic_tokens(teacher_topics,method="freq")
