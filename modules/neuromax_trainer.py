...
class BasicTrainer:
    ...
    def export_top_words(self, vocab, num_top_words=10):
        topics = self.export_beta()
        print(topics.shape)
        topics = topics.argsort(axis=1)[:, ::-1]
        # top 10 words
        topics = topics[:, :num_top_words]
        top_tokens = [[vocab[i] for i in topic] for topic in topics]
        return top_tokens
